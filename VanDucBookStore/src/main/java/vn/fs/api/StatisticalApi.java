...
public class StatisticalApi {
	...
	public ResponseEntity<List<CategoryBestSeller>> getCategoryBestSeller() {
		...
		for (int i = 0; i < list.size(); i++) {
			int count = list.get(i)[0] != null ? ((Number) list.get(i)[0]).intValue() : 0;
			String name = list.get(i)[1] != null ? String.valueOf(list.get(i)[1]) : "";
			Double amount = list.get(i)[2] != null ? ((Number) list.get(i)[2]).doubleValue() : 0.0;
			CategoryBestSeller categoryBestSeller = new CategoryBestSeller(name, count, amount);
