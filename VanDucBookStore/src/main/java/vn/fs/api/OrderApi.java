...
public class OrderApi {
	...
	@Autowired
	NotificationWebSocketHandle notificationWebSocketHandle;
	...
	public ResponseEntity<Order> checkout(@PathVariable("email") String email, @RequestBody Cart cart,
			@org.springframework.web.bind.annotation.RequestParam(value = "paymentMethod", defaultValue = "0") int paymentMethod) {
		if (!userRepository.existsByEmail(email)) {
			return ResponseEntity.notFound().build();
		}
...
//		cartDetailRepository.deleteByCart(cart);
		...
		try {
			...
			notification.setMessage("đã đặt một đơn hàng mới.");
			notification.setTime(new Date());
			notification.setStatus(false);
			notificationRepository.save(notification);

			String userName = order.getUser() != null ? order.getUser().getName() : email;
			String wsPayload = "{\"user\":\"" + userName + "\",\"message\":\"đã đặt một đơn hàng mới.\"}";
	...
	public ResponseEntity<Void> cancel(@PathVariable("orderId") Long id) {
		...
		try {
			String wsPayload = "{\"user\":\"Hệ thống\",\"message\":\"Đơn hàng #" + id + " đã được cập nhật.\"}";
			notificationWebSocketHandle.broadcast(wsPayload);
		} catch (Exception e) {
			e.printStackTrace();
		}
